...
| `surface-swift` | surface | 1.1.0 | The Swift storefront content surface (born in the base split): both content areas (EN + NL), the entire merge tree, `UrlPath`, and its own 128 Swift v2.4.0 item-type XMLs (self-contained). 1.1.0 ships the product short/long descriptions in the product-page composition (buy-panel teaser + full-width "Overview", both areas). 1.0.1 added the durable image-crop normalization + the List-mode PLP card (SKU/price/stock/qty/add-to-cart). Proven on DW 10.28.1-PreRelease. |
