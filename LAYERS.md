...
| `sample-data` | sample-data | 2.1.0 | Demo identities (buyer/CSR) + the demo product catalog (EcomProducts 20 / EcomGroups 3) + contract pricing, shipped as SQL under `merge/_sql/`. 2.1.0 renames the catalogue off its "Fixture ..." test-fixture names onto function-descriptive placeholders (`Sample Product NN — <role>` / `Sample Group N — <function>`) and adds `ProductShortDescription` (reserved keys, product numbers, prices and row counts unchanged). 2.0.2 seeds the delivered order + RMA link for `feature-rma`. Activated when an edition sets `sampleData: true`. |
