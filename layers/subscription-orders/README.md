# subscription-orders — Pack #2: Subscription Orders

Feature pack delivering invoice-based recurring (subscription) checkout and a customer-center
subscriptions surface on top of the swift/2.3 baseline. Satisfies
[`docs/pack-contract.md`](../../docs/pack-contract.md) in full (schema, fragment rules including
content, collision checks, activation manifest, publish guards). **Zero custom `.cs`** — the
native DW 10.26.9 engine does all the work; the pack is pure exposure wiring.

## What the pack delivers

| Capability | How | Artifact |
|------------|-----|----------|
| Invoice recurring checkout (PK2-01) | Pack-owned Subscribe page with a minimized 3-step `eCom_CartV2` paragraph; the pack step template renders the native recurring panel (`EcomRecurringOrderCreate` + interval fields — decompile-verified names). The engine's `DefaultCheckoutHandler` (auto-resolved for the gateway-less PAY2 Invoice method) implements `IRecurring`, so checkout completion places the base order AND the `EcomRecurringOrder` template row | Subscribe page fragment (`/swift-2/subscribe`), `templates/Designs/Swift-v2/eCom7/CartV2/Step/PackSubscribeCheckout.cshtml` |
| Customer-center subscriptions list + manage (PK2-02) | Pack-owned Subscriptions page using the stock `eCom_CustomerExperienceCenter` app with `<OrderType>Recurringorder</OrderType>` (mandatory — the standard Orders list SQL excludes recurring templates) and a pack list template with interval/next-delivery detail and an ownership-guarded end-subscription action. The page sits under the base **Customer center → Account** nav tree (beside Orders/Favorites/etc., navigationTag `PackSubscriptionsPage`); because `Account` is base **deploy**-owned content, this page ships as a **deploy** fragment (`fragmentModes: ["seed","deploy"]`) so the base Account parent is staged at deserialize | Subscriptions page fragment under `/Customer center/Account/Subscriptions`, `templates/Designs/Swift-v2/eCom/CustomerExperienceCenter/Orders/List/PackSubscriptions_List.cshtml` |
| Declared config dependencies (PK2-03) | SQL EXISTS probes on the baseline rows the pack rides on: `EcomPayments` PAY2 (Invoice, ENU), `EcomShippings` SHIP9 (Home delivery, ENU), and the native `ScheduledTask` row "Place recurring orders" | `pack.json` configRows |

## Zero-.cs architecture

`csLedger: []` — this pack ships **zero custom `.cs`**: templates + content fragments +
configRows, plus (since 1.1.0) its own pack-owned catalog rows. `DefaultCheckoutHandler` covers
...
**declared configRow dependencies, not fragment rows** — the pack inserts nothing into those
tables. **1.1.0 self-sufficiency:** the baseline is scaffolding-only, so the pack now ships its
...
the checkout-recurring probe rides the pack product, never a base catalog row.
...
Do NOT flip `TaskEnabled` via SQL — DW caches scheduled-task state at startup, and pack
...
(`tests/Pack.Tests.ps1`, Pitfall 4 single-input lock).
...
Any `Invoke-Serialize` run against a host with this pack ACTIVE captures the Subscribe and
Subscriptions pages into the base baseline content tree, silently breaking the base/pack
ownership split.
