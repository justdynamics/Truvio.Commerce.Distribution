...
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0006' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A completeness score is what a rule set returns when it measures a record against the fields its data model says it must carry. This record sits at the low end of the scale, the state a freshly imported product arrives in before anyone has enriched it. The score moves as fields are filled, which is what drives a product between the enrichment queries a PIM team works from.' WHERE ProductId = 'TCPROD0006' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0007' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A completeness score is what a rule set returns when it measures a record against the fields its data model says it must carry. This record sits mid-scale and is in review, the state most of the catalogue lives in on any working day. The score moves as fields are filled, which is what drives a product between the enrichment queries a PIM team works from.' WHERE ProductId = 'TCPROD0007' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0008' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A completeness score is what a rule set returns when it measures a record against the fields its data model says it must carry. This record is approved and near the top of the scale, which is what a completion rule set is aiming a product at. The score moves as fields are filled, which is what drives a product between the enrichment queries a PIM team works from.' WHERE ProductId = 'TCPROD0008' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0009' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A completeness score is what a rule set returns when it measures a record against the fields its data model says it must carry. This record carries a partially filled category, so the gap between what is present and what the rule asks for is visible on the page. The score moves as fields are filled, which is what drives a product between the enrichment queries a PIM team works from.' WHERE ProductId = 'TCPROD0009' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0010' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A completeness score is what a rule set returns when it measures a record against the fields its data model says it must carry. This record is the comparison row: the same rule set, a different field mix, and therefore a different score. The score moves as fields are filled, which is what drives a product between the enrichment queries a PIM team works from.' WHERE ProductId = 'TCPROD0010' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0011' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A workflow state is a named position in the enrichment lifecycle, and the transitions between states are what an editorial team hands work over on. This record is in the draft state, the entry point of the flow, where fields may still be empty. States are data, not code, so a solution renames them, adds them or gates them on a completeness score without a deployment.' WHERE ProductId = 'TCPROD0011' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0012' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A workflow state is a named position in the enrichment lifecycle, and the transitions between states are what an editorial team hands work over on. This record is in review, which is the state a transition hands to a second pair of eyes. States are data, not code, so a solution renames them, adds them or gates them on a completeness score without a deployment.' WHERE ProductId = 'TCPROD0012' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0013' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A workflow state is a named position in the enrichment lifecycle, and the transitions between states are what an editorial team hands work over on. This record is a service line rather than a stock item: no stock, no shipment, and it prices and renders like any other row. States are data, not code, so a solution renames them, adds them or gates them on a completeness score without a deployment.' WHERE ProductId = 'TCPROD0013' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0014' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A workflow state is a named position in the enrichment lifecycle, and the transitions between states are what an editorial team hands work over on. This record is published, the state a storefront query filters on when it decides what a shopper may reach. States are data, not code, so a solution renames them, adds them or gates them on a completeness score without a deployment.' WHERE ProductId = 'TCPROD0014' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0015' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A workflow state is a named position in the enrichment lifecycle, and the transitions between states are what an editorial team hands work over on. This record has been sent back a step, which is the transition an editorial flow needs as much as the forward one. States are data, not code, so a solution renames them, adds them or gates them on a completeness score without a deployment.' WHERE ProductId = 'TCPROD0015' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
...
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0031' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A page node is one position in the content tree, carrying its own item type, its own field values and its own grid of rows. This record stands for a top-level node, the kind that becomes a primary navigation entry. The tree is the navigation, the URL structure and the permission surface at once, which is why moving a node moves all three together.' WHERE ProductId = 'TCPROD0031' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0032' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A page node is one position in the content tree, carrying its own item type, its own field values and its own grid of rows. This record stands for a child node, reachable through its parent and carrying its own grid. The tree is the navigation, the URL structure and the permission surface at once, which is why moving a node moves all three together.' WHERE ProductId = 'TCPROD0032' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0033' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A page node is one position in the content tree, carrying its own item type, its own field values and its own grid of rows. This record stands for a node that is live but hidden from the menu, which is how landing pages are usually published. The tree is the navigation, the URL structure and the permission surface at once, which is why moving a node moves all three together.' WHERE ProductId = 'TCPROD0033' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0034' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A page node is one position in the content tree, carrying its own item type, its own field values and its own grid of rows. This record stands for a node whose item type differs from its siblings'', so the same tree carries more than one page shape. The tree is the navigation, the URL structure and the permission surface at once, which is why moving a node moves all three together.' WHERE ProductId = 'TCPROD0034' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0035' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A page node is one position in the content tree, carrying its own item type, its own field values and its own grid of rows. This record is the comparison row: a plain node with nothing special about it, which is most of any real tree. The tree is the navigation, the URL structure and the permission surface at once, which is why moving a node moves all three together.' WHERE ProductId = 'TCPROD0035' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0036' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A paragraph is the unit a page is actually built from: a typed content block with declared fields and a template that renders them. This record stands for a text block, the most common paragraph on any site. Editors compose pages from these blocks in the Visual Editor, and a new block type is an item-type declaration plus one template.' WHERE ProductId = 'TCPROD0036' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0037' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A paragraph is the unit a page is actually built from: a typed content block with declared fields and a template that renders them. This record stands for a media block, where the field list is an asset reference plus its presentation settings. Editors compose pages from these blocks in the Visual Editor, and a new block type is an item-type declaration plus one template.' WHERE ProductId = 'TCPROD0037' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0038' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A paragraph is the unit a page is actually built from: a typed content block with declared fields and a template that renders them. This record stands for an app paragraph, which hosts a commerce or forms application inside the page grid. Editors compose pages from these blocks in the Visual Editor, and a new block type is an item-type declaration plus one template.' WHERE ProductId = 'TCPROD0038' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0039' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A paragraph is the unit a page is actually built from: a typed content block with declared fields and a template that renders them. This record stands for a repeatable block, where one paragraph carries a list of child items. Editors compose pages from these blocks in the Visual Editor, and a new block type is an item-type declaration plus one template.' WHERE ProductId = 'TCPROD0039' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0040' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A paragraph is the unit a page is actually built from: a typed content block with declared fields and a template that renders them. This record is the comparison row: the same grid position with the plainest block in it. Editors compose pages from these blocks in the Visual Editor, and a new block type is an item-type declaration plus one template.' WHERE ProductId = 'TCPROD0040' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0041' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'An item type is the schema behind a page, a paragraph or a record - the field list, the editors and the validation a content author meets. This record stands for a page-level schema, the field list a page author fills in. Because the schema is declared and versioned, the same type serves the backend editor, the rendered page and the delivery API without restating itself.' WHERE ProductId = 'TCPROD0041' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0042' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'An item type is the schema behind a page, a paragraph or a record - the field list, the editors and the validation a content author meets. This record stands for a paragraph-level schema, which is the same mechanism scoped to a block. Because the schema is declared and versioned, the same type serves the backend editor, the rendered page and the delivery API without restating itself.' WHERE ProductId = 'TCPROD0042' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0043' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'An item type is the schema behind a page, a paragraph or a record - the field list, the editors and the validation a content author meets. This record stands for a schema carrying a relation field, so one record points at another. Because the schema is declared and versioned, the same type serves the backend editor, the rendered page and the delivery API without restating itself.' WHERE ProductId = 'TCPROD0043' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0044' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'An item type is the schema behind a page, a paragraph or a record - the field list, the editors and the validation a content author meets. This record stands for a schema carrying a list field, where the editor is a repeater rather than a single input. Because the schema is declared and versioned, the same type serves the backend editor, the rendered page and the delivery API without restating itself.' WHERE ProductId = 'TCPROD0044' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0045' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'An item type is the schema behind a page, a paragraph or a record - the field list, the editors and the validation a content author meets. This record is the comparison row: the minimal schema, two fields and no editors beyond the defaults. Because the schema is declared and versioned, the same type serves the backend editor, the rendered page and the delivery API without restating itself.' WHERE ProductId = 'TCPROD0045' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0046' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A product group is both a node in the catalogue tree and a membership set, and a product may belong to several while one relation stays primary. This record sits in one group only, with that relation primary, which is the plain case. The primary relation decides the canonical URL and the breadcrumb; the secondary ones decide where else the product is reachable.' WHERE ProductId = 'TCPROD0046' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0047' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A product group is both a node in the catalogue tree and a membership set, and a product may belong to several while one relation stays primary. This record sits in a subgroup and in its parent group, which is what keeps a top-level list from rendering empty. The primary relation decides the canonical URL and the breadcrumb; the secondary ones decide where else the product is reachable.' WHERE ProductId = 'TCPROD0047' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0048' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A product group is both a node in the catalogue tree and a membership set, and a product may belong to several while one relation stays primary. This record sits in two unrelated groups, so it is reachable from two branches of the tree under one canonical URL. The primary relation decides the canonical URL and the breadcrumb; the secondary ones decide where else the product is reachable.' WHERE ProductId = 'TCPROD0048' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0049' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A product group is both a node in the catalogue tree and a membership set, and a product may belong to several while one relation stays primary. This record sits deep in the tree, which is where breadcrumb and menu depth stop agreeing unless the relations are right. The primary relation decides the canonical URL and the breadcrumb; the secondary ones decide where else the product is reachable.' WHERE ProductId = 'TCPROD0049' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0050' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A product group is both a node in the catalogue tree and a membership set, and a product may belong to several while one relation stays primary. This record is the comparison row: one group, no secondary relation, nothing inherited. The primary relation decides the canonical URL and the breadcrumb; the secondary ones decide where else the product is reachable.' WHERE ProductId = 'TCPROD0050' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0051' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A permission grant binds a subject - a user or a user group - to what it may see and do on a page, a record or a commerce surface. This record stands for a grant made directly to a user. Grants compose rather than override, so an account''s effective access is the union of its memberships and is auditable row by row.' WHERE ProductId = 'TCPROD0051' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0052' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A permission grant binds a subject - a user or a user group - to what it may see and do on a page, a record or a commerce surface. This record stands for a grant made to a group, which is how access is actually administered at any size. Grants compose rather than override, so an account''s effective access is the union of its memberships and is auditable row by row.' WHERE ProductId = 'TCPROD0052' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0053' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A permission grant binds a subject - a user or a user group - to what it may see and do on a page, a record or a commerce surface. This record is a service line rather than a stock item: no stock, no shipment, and it prices and renders like any other row. Grants compose rather than override, so an account''s effective access is the union of its memberships and is auditable row by row.' WHERE ProductId = 'TCPROD0053' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0054' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A permission grant binds a subject - a user or a user group - to what it may see and do on a page, a record or a commerce surface. This record stands for a grant that is scoped to a single commerce surface rather than to the whole solution. Grants compose rather than override, so an account''s effective access is the union of its memberships and is auditable row by row.' WHERE ProductId = 'TCPROD0054' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0055' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'A permission grant binds a subject - a user or a user group - to what it may see and do on a page, a record or a commerce surface. This record is the comparison row: no grant on it, so the difference between granted and ungranted is visible side by side. Grants compose rather than override, so an account''s effective access is the union of its memberships and is auditable row by row.' WHERE ProductId = 'TCPROD0055' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0056' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'Impersonation lets an authorised account act as another user, on that user''s own prices, assortments and order history, without knowing a password. This record stands for the operator side of the session - the account that opens it and stays accountable for it. Every impersonated session stays attributed to the operator who opened it, which is what makes it a support tool rather than a shared login.' WHERE ProductId = 'TCPROD0056' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0057' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'Impersonation lets an authorised account act as another user, on that user''s own prices, assortments and order history, without knowing a password. This record stands for the subject side - the account whose prices and history the session renders. Every impersonated session stays attributed to the operator who opened it, which is what makes it a support tool rather than a shared login.' WHERE ProductId = 'TCPROD0057' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0058' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'Impersonation lets an authorised account act as another user, on that user''s own prices, assortments and order history, without knowing a password. This record stands for a session scoped to a single account rather than to a whole customer group. Every impersonated session stays attributed to the operator who opened it, which is what makes it a support tool rather than a shared login.' WHERE ProductId = 'TCPROD0058' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0059' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'Impersonation lets an authorised account act as another user, on that user''s own prices, assortments and order history, without knowing a password. This record stands for a session opened from the backend, which is the path a support agent actually takes. Every impersonated session stays attributed to the operator who opened it, which is what makes it a support tool rather than a shared login.' WHERE ProductId = 'TCPROD0059' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
IF EXISTS (SELECT 1 FROM EcomProducts WHERE ProductId = 'TCPROD0060' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '')
    UPDATE EcomProducts SET ProductLongDescription = N'Impersonation lets an authorised account act as another user, on that user''s own prices, assortments and order history, without knowing a password. This record is the comparison row: the same account with no session open on it. Every impersonated session stays attributed to the operator who opened it, which is what makes it a support tool rather than a shared login.' WHERE ProductId = 'TCPROD0060' AND ProductVariantId = '' AND ProductLanguageId = 'ENU' AND ISNULL(ProductLongDescription, '') = '';
-- ---------------------------------------------------------------------------
